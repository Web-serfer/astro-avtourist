...
function generatePost(post){
  const returnPostDate = (date) => `${
    ['Январь', 'Февраль', 'Март', 'Апрель', 'Май', 'Июнь', 'Июль', 'Август', 'Сентябрь', 'Октябрь', 'Ноябрь', 'Декабрь'][date.getMonth()]} ${date.getDate()}, ${date.getFullYear()}`;
  ...
  <div class="post__author">
    ...
    <div>
      ...
      <p class="post__author--name">${post.user.name[0].firstDescr} ${post.user.name[1].lastDescr}</p>
    </div>
  </div>
  </div>
  ...
  <a href="${post.meta.url}" class="btn">Читать пост ...</a>
  `;
  return article;
}
